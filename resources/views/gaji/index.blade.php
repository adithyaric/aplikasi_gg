...
@push('js')
    ...
    `);
            ...
            $.ajax({
                url: '{{ route('gaji.period-detail', ['periode_tahun' => '__tahun__', 'periode_bulan' => '__bulan__']) }}'
                    .replace('__tahun__', tahun).replace('__bulan__', bulan),
                method: 'GET',
                success: function(response) {
                    if (response.success) {
                        $('#modalDetailLabel').text('Detail Gaji Periode ' + response.periode);
    ...
    <script>
        $(document).on('click', '.btn-bulk-confirm', function() {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        url: '{{ route('gaji.bulk-confirm', ['periode_tahun' => '__tahun__', 'periode_bulan' => '__bulan__']) }}'
                            .replace('__tahun__', tahun).replace('__bulan__', bulan),
                        method: 'POST',
                        data: {
                            _token: "{{ csrf_token() }}"
