            ...
            <div class="row">
                <div class="col-md-12">
                    <div class="flex-wrap d-flex justify-content-between align-items-center">
                        ...
                        <div>
                            <a href="{{ route('paketmenu.create') }}" class="btn btn-link btn-soft-light">
        ...
        <div class="row">
            <div class="col-12">
                <div class="card">
                    ...
                    <div class="card-body">
                        <form action="{{ route('import.paket-menu') }}" method="POST" enctype="multipart/form-data"
                            class="row g-3">
                            @csrf
                            <div class="col-md-3">
                            ...
                            <div class="col-md-9">
                                ...
                                <a href="{{ asset('templates/template-paket-menu.csv') }}" class="btn btn-secondary ms-2">
                    ...
                    <div class="card-body">
                        <div class="table-responsive text-nowrap">
                            <table class="table table-bordered table-sm" id="tablePaketMenu">
                                ...
                                <tbody>
                                    @foreach ($paketmenus as $index => $paket)
                                        @php
                                            $menuCount = $paket->menus->count();
                                            $firstMenu = true;
                                        @endphp
                                        @foreach ($paket->menus as $menuIndex => $menu)
                                            @php
                                                $bahanCount = $menu->bahanBakus->count();
                                                $firstBahan = true;
                                            @endphp
                                            @foreach ($menu->bahanBakus as $bahanIndex => $bahan)
                                                <tr>
                                                    ...
                                                    @endif
                                                    @if ($firstBahan)
                                                        <td rowspan="{{ $bahanCount }}">{{ $menu->nama }}</td>
                                                    @endif
                                                    <td>{{ $bahan->nama }}</td>
                                                    @if ($firstMenu && $firstBahan)
                                                        <td class="text-center"
                                                            ...
                                                            <button type="button" class="btn btn-sm btn-danger"
                                                                onclick="deletePaketMenu({{ $paket->id }})">
                                                                <span class="btn-inner">
                                                                    <svg class="icon-20" width="20" viewBox="0 0 24 24"
                                                                        fill="none" xmlns="http://www.w3.org/2000/svg">
                                                                        <path fill-rule="evenodd" clip-rule="evenodd"
